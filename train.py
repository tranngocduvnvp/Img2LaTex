from img2tex.dataset.dataset import Im2LatexDataset
import os
import argparse
import logging
import yaml
...
from img2tex.eval import evaluate
from img2tex.models import get_model
# from img2tex.utils import *
from img2tex.utils import in_model_path, parse_args, seed_everything, get_optimizer, get_scheduler, gpu_memory_check
...
def train(args):
    ...
    def save_models(e, step=0):
        torch.save(model.state_dict(), os.path.join("/content/drive/MyDrive/ScienceOCR/", '%snew_e%02d_step%02d.pth' % (args.name, e+1, step)))
        yaml.dump(dict(args), open(os.path.join("/content/drive/MyDrive/ScienceOCR/", 'config.yaml'), 'w+'))
