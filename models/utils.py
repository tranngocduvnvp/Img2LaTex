...
class Model(nn.Module):
    ...
    def generate(self, x: torch.Tensor, temperature: float = 0.25):
        return self.decoder.generate((torch.LongTensor([self.args.bos_token]*x.shape[0])[:, None]).to(x.device), self.args.max_seq_len,
                                     eos_token=self.args.eos_token, context=self.encoder(x), temperature=temperature)

class Model4Serve(nn.Module):
    def __init__(self, encoder, decoder, args):
        super().__init__()
        self.encoder = encoder
        self.decoder = decoder
    ...
    @torch.no_grad()
    def forward(self, x: torch.Tensor, temperature: float = 0.25):
...
def get_model4serve(args):
    # print(args.encoder_structure.lower())
    ...
    model = Model4Serve(encoder, decoder, args)
    if args.wandb:
        import wandb
        wandb.watch(Model4Serve)
